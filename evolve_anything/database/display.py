...
class DatabaseDisplay:
    ...
    def print_program_summary(self, program, console: Optional[RichConsole] = None):
        ...
        table.add_column("Tokens (In/Out)", style="green", justify="right", width=15)
        ...
        if program.combined_score is not None:
            ...
            if not program.correct:
                ...
                # For minimization, we don't assume a scale, so use a neutral color
                color = "bold cyan"
            else:
                if score > 0.8:
                    color = "bold green"
        ...
        # Format token count
        tokens_display = "[dim]N/A[/dim]"
        if program.metadata:
            total_tokens = int(program.metadata.get("total_tokens", 0))
            input_tokens = int(program.metadata.get("input_tokens", 0))
            output_tokens = int(program.metadata.get("output_tokens", 0))
            ...
            if input_tokens > 0 or output_tokens > 0:
                ...
                tokens_display = f"{fmt_k(input_tokens)}/{fmt_k(output_tokens)}"
            elif total_tokens > 0:
    ...
    def print_summary(self, console: Optional[RichConsole] = None) -> None:
        ...
        # Calculate total tokens from metadata of all programs
        total_tokens = 0
        total_input_tokens = 0
        total_output_tokens = 0
        total_compute_time = 0.0
        avg_score = 0.0
        # Initialize best_score based on optimization direction
            ...
            for row in self.cursor.fetchall():
                if row["metadata"]:
                    ...
                    if "output_tokens" in metadata:
                        total_output_tokens += int(metadata["output_tokens"])
        ...
        # If no scores found, reset best_score to 0.0 for display
        ...
        # Table 3: Token & Stats Summary
        stats_table = RichTable(
            title="[bold magenta]Token & Stats Summary[/bold magenta]",
            border_style="magenta",
            box=rich.box.ROUNDED,
            width=40,  # Set width for consistent layout
        )
        stats_table.add_column("Metric", style="magenta bold")
        stats_table.add_column("Value", style="green")

        # Add token information
        if total_tokens > 0:
            if total_tokens >= 1000000:
                tokens_str = f"{total_tokens / 1000000:.2f}M"
            elif total_tokens >= 1000:
                tokens_str = f"{total_tokens / 1000:.1f}k"
            else:
                tokens_str = str(total_tokens)
            stats_table.add_row("Total Tokens", f"[bold]{tokens_str}[/bold]")

            if total_input_tokens > 0 or total_output_tokens > 0:
                if total_input_tokens >= 1000000:
                    in_str = f"{total_input_tokens / 1000000:.2f}M"
                elif total_input_tokens >= 1000:
                    in_str = f"{total_input_tokens / 1000:.1f}k"
                else:
                    in_str = str(total_input_tokens)

                if total_output_tokens >= 1000000:
                    out_str = f"{total_output_tokens / 1000000:.2f}M"
                elif total_output_tokens >= 1000:
                    out_str = f"{total_output_tokens / 1000:.1f}k"
                else:
                    out_str = str(total_output_tokens)

                stats_table.add_row("  Input Tokens", in_str)
                stats_table.add_row("  Output Tokens", out_str)

            if total_programs > 0:
                avg_tokens = total_tokens // total_programs
                stats_table.add_row("Avg Tokens/Program", f"{avg_tokens}")
        ...
        if total_compute_time > 0:
            ...
            stats_table.add_row("Total Compute", time_str)

        # Add score stats if available
        if num_with_scores > 0:
            avg_score_display = f"{avg_score / num_with_scores:.2f}"
            stats_table.add_row("Avg Score", avg_score_display)
            stats_table.add_row("Best Score", f"[bold]{best_score:.2f}[/bold]")
            stats_table.add_row("Median Score", f"{median_score:.2f}")
        ...
        tables_to_display.append(stats_table)
        ...
        query = (
            ...
            f"ORDER BY combined_score {self.sort_order} LIMIT 10"
        )
        self.cursor.execute(query)
    ...
    def print_sampling_summary(
        ...
        table.add_column("Tokens", style="green", justify="right", width=9)
        ...
        def format_program_row(prog, role_name):
            ...
            tokens_display = "[dim]N/A[/dim]"
            if prog.metadata:
                total_tokens = int(prog.metadata.get("total_tokens", 0))
                if total_tokens > 0:
                    if total_tokens >= 1000:
                        tokens_display = f"{total_tokens / 1000:.1f}k"
                    else:

...
from evolve_anything.llm.embedding import EmbeddingClient
...
class DatabaseConfig:
    ...
    parent_selection_strategy: str = (
        "power_law"  # "weighted"/"power_law"/"beam_search"/"best_of_n"/"map_elites"
    )

    # Power-law parent selection parameters
    exploitation_alpha: float = (
        1.0  # Power law exponent for archive sampling (0=uniform, higher=more greedy)
    )
    exploration_alpha: float = (
        0.5  # Power law exponent for exploration (0=uniform, higher=more greedy)
    )
    exploitation_ratio: float = (
        0.2  # Chance to pick from archive vs all correct programs
    )
    ...
    map_elites_config: Optional[MapElitesConfig] = None
...
class ProgramDatabase:
    ...
    def __init__(
        ...
        # Store minimize setting from config
        self._minimize: bool = getattr(config, "minimize", False)

        # Initialize island manager (will be set after db connection)
        self.island_manager: Optional[CombinedIslandManager] = None

        # Initialize MAP-Elites archive (will be set after db connection)
        self.map_elites_archive: Optional[MapElitesArchive] = None
        self._map_elites_config: Optional[MapElitesConfig] = getattr(
            config, "map_elites_config", None
        )
        ...
        # Initialize MAP-Elites archive if configured
        if self._map_elites_config is None:
            # Create default MAP-Elites config for code evolution
            self._map_elites_config = MapElitesConfig.default_for_code()

        if self._map_elites_config.enabled and self.cursor and self.conn:
            self.map_elites_archive = MapElitesArchive(
                cursor=self.cursor,
                conn=self.conn,
                config=self._map_elites_config,
                minimize_score=self._minimize,
            )
            logger.info(
                f"MAP-Elites archive initialized with "
                f"{len(self._map_elites_config.dimensions)} behavioral dimensions"
            )
    ...
    def sort_order(self) -> str:
        ...
        return "ASC" if self._minimize else "DESC"
    ...
    def add(self, program: Program, verbose: bool = False) -> str:
        ...
        # Update MAP-Elites behavioral archive
        if self.map_elites_archive is not None:
            added_to_me = self.map_elites_archive.add(program, verbose=verbose)
            if added_to_me and verbose:
                logger.info(
                    f"Program {program.id[:8]}... added/updated in MAP-Elites archive"
                )
    ...
    def _is_better(self, program1: Program, program2: Program) -> bool:
        """Check if program1 is better than program2.

        Respects self.minimize setting: when minimizing, lower scores are better.
        """
        ...
        if s1 is not None and s2 is not None:
            if s1 != s2:
                ...
                return s1 < s2 if self._minimize else s1 > s2
        elif s1 is not None:
        ...
        try:
            ...
            if avg1 != avg2:
                return avg1 < avg2 if self._minimize else avg1 > avg2
        except Exception:
            return False

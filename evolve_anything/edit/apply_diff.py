...
_EVOLVE_START_MARKER = format_config.DEFAULT_FORMAT.evolve_block_start
_EVOLVE_END_MARKER = format_config.DEFAULT_FORMAT.evolve_block_end
EVOLVE_START = re.compile(rf"(?:#|//|)?\s*{_EVOLVE_START_MARKER}")
EVOLVE_END = re.compile(rf"(?:#|//|)?\s*{_EVOLVE_END_MARKER}")
...
def _clean_evolve_markers(text: str) -> str:
    ...
    start_marker = format_config.DEFAULT_FORMAT.evolve_block_start
    end_marker = format_config.DEFAULT_FORMAT.evolve_block_end
    patterns_to_remove = [
        rf"^\s*#\s*{start_marker}\s*$",  # Python style
        rf"^\s*//\s*{start_marker}\s*$",  # C/C++/CUDA style
        rf"^\s*{start_marker}\s*$",  # Plain text
        rf"^\s*#\s*{end_marker}\s*$",  # Python style
        rf"^\s*//\s*{end_marker}\s*$",  # C/C++/CUDA
        rf"^\s*{end_marker}\s*$",  # Plain text
    ]
    ...
    if similar_lines:
        ...
        error_parts.extend(
            [
                ...
                f"• Check that you're searching within {format_config.DEFAULT_FORMAT.evolve_block_comment} regions",
...
def _create_evolve_block_error(
    ...
    error_parts = [
        f"Attempted to edit outside {format_config.DEFAULT_FORMAT.evolve_block_comment} regions",
        "",
    ...
    error_parts.extend(
        [
            ...
            "",
            f"Available editable regions ({format_config.DEFAULT_FORMAT.evolve_block_comment} content):",
        ]
    )
    ...
    if mutable_ranges:
        ...
        error_parts.append(
            f"  No {format_config.DEFAULT_FORMAT.evolve_block_comment} regions found in the code!"
        )
...
def apply_diff_patch(
    ...
    suffix = f".{format_config.get_file_extension(language)}"
